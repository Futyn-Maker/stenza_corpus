...
Веб-адрес: https://stenza.fikl.ru

Инструкция пользователя

Сайт поддерживает четыре вида поиска: 

- По словоформе: для такого поиска надо ввести в поле поиска интересующую словоформу в кавычках.

Например: «Доктор»

- По лемме без специфицированной части речи: для такого поиска надо ввести в поисковую строку слово без кавычек в любой его форме

Например: Роза

- По частеречному тегу: для этого введите его в формате UD (заглавными буквами)

Например: NOUN (существительное)

- По лемме со специфицированной частью речи: для такого поиска надо ввести в поисковую строку запрос вида «слово+часть речи». При этом на позицию слова необходимо вводить начальную форму интересующего слова.
...
Например: не знать что; 

Например: кошка+NOUN VERB
...
При разметке мы пользовались тегами [Universal Dependencies](https://universaldependencies.org), в частности, регламентом [Taiga](https://github.com/UniversalDependencies/UD_Russian-Taiga) – подробнее о них можно прочитать по ссылкам.
...
Склонируйте репозиторий: git clone https://github.com/Futyn-Maker/stenza_corpus.git

Перейдите в папку с корпусом: cd stenza_corpus/app

Установите зависимости, их немного: pip3 install -r requirements.txt

Запустите сайт: python3 app.py

Ищите сайт на http://127.0.0.1:5000
...
В конечном счете в нашем корпусе получилось 95 текстов, 25558 предложений и 213377 токенов.
...
Credits
 ...
 - Дарья Харламова - выбор корпуса, выбор парсера, чистка текстов, запись метаданных, оформление сайта, описание проекта
 - Андрей Якубой - изготовление фласк-приложения, хостинг, сбор проекта в единое целое, общий Code Review, оформление сайта, описание проекта
